...
class SocketTest {
    ...
    void netstat() {
        ...
        // Mac netstat
        Assertions.assertEquals(0, execPipe("netstat -nat -f inet -p tcp | grep -E 'tcp4[^6]*LISTEN'").waitFor());
    }
    ...
    void Address_already_in_use() {
        ...
        log.error("BindException", exception);
        Assertions.assertEquals(0, execPipe("netstat -nat -f inet -p tcp | grep -E 'tcp4.*" + port + "'").waitFor());
    }
    ...
    void analyseConnectionRefused() {
        int port = 1000;
        Assertions.assertNotEquals(0, execPipe("netstat -nat -f inet -p tcp | grep -E 'tcp4[^6]" + port).waitFor());
    ...
    void bindHostByNCAll() {
        ...
        Assertions.assertEquals(0, execPipe("netstat -nat -f inet -p tcp | grep -E '" + port + "'").waitFor());
        process.destroy();
    }
    ...
    void backlog() {
        ...
        Process process = execPipe(sudoPipe("tcpdump -n -i lo0 -c 7 tcp and port " + port));
